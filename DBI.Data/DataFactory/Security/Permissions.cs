...
namespace DBI.Data
{
    public partial class SYS_PERMISSIONS
    {
        ...
        public static List<SYS_PERMISSIONS> GetPermissionsHierarchy()
        {
            using (Entities _context = new Entities())
            {
                string Query = "select permission_id, permission_name, parent_perm_id from xxems.sys_permissions connect by prior permission_id = parent_perm_id start with permission_id=1";
